(ns gijiroku.cacao
  "Agent-side CACAO issuance (JVM). The gijiroku actor mints its OWN
  ...
  `kotoba.cacao` (keep in sync — same copy kekkai.cacao carries), and the
...
;; ───────── key-derived IPNS graph name (per-actor graph = self key) ─────────
...
(defn generate-identity
  ...
  test bootstrap — a provisioned actor persists and reloads its key."
  []
...
(defn load-or-create-identity!
  "Per-actor key: load the actor's persisted Ed25519 identity at `path`, or
