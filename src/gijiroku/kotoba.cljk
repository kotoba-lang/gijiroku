(ns gijiroku.kotoba
  ...
  that map against the remote pod, so this is purely a constructor. Mirrors
  kekkai.kotoba.
  ...
  itself is already proven backend-agnostic (test/gijiroku/store_contract_test)."
  (:require [kotoba.lang.text :as str]
            [langchain.kotoba-db :as kdb]
            [gijiroku.cacao :as cacao]
...
(defn kotoba-store
  "A `store/DatomicStore` backed by a kotoba-server pod (e.g. kotobase.net).
   Auth options (pick one):
     ...
     :identity         a gijiroku.cacao identity ({:private-key :public-key :did})
                       → the actor SELF-MINTS a CACAO for :grant (default a
                       read grant on :graph). No handed token, no
                       coordination-server auth-key — the actor issues its own
                       capability from its own key.
   opts:
