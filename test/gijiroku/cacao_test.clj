(ns gijiroku.cacao-test
  "Offline verification of agent-side CACAO minting — mirrors kekkai.cacao-test.
  Server acceptance can't be tested here, but the crypto + encoding are fully
...
(deftest minted-cacao-signature-verifies
  (testing "the Ed25519 signature is over the exact SIWE message"
    (let [id    (c/generate-identity)
          grant {:cap :cap/read :scope "gijiroku"}
          opts  {:aud "https://kotobase.net" :nonce "n1" :issued-at "2026-07-03T00:00:00Z"}
...
(deftest minted-cacao-is-wellformed-cbor
  (let [id    (c/generate-identity)
        cacao (c/mint id {:cap :cap/transact :scope "gijiroku"}
                      {:aud "https://kotobase.net" :nonce "n2"
                       :issued-at "2026-07-03T00:00:00Z" :expiry "2026-07-03T01:00:00Z"})
...
(deftest per-actor-key-persists
  (testing "load-or-create! generates once, then reloads the same identity"
    (let [p  (str (System/getProperty "java.io.tmpdir") "/gijiroku-id-test-" (System/nanoTime) ".edn")
...
(deftest resources-encode-capability-and-graph
  (is (= ["kotoba://op/datom:read" "kotoba://graph/gijiroku"]
         (c/grant->resources {:cap :cap/read :scope "gijiroku"})))
